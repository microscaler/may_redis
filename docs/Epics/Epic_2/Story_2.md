...
- `src/codec/mod.rs` — `pub struct RESPReader { buf: BytesMut, pos: usize }`
- `src/codec/reader.rs` — implementation
...
- [x] Define `RESPReader` with `buf: BytesMut` and `pos: usize`
- [x] Implement `new(buf)` constructor
- [x] Implement `read_line()` — reads until `\r\n`, returns the line content
- [x] Implement `read_bytes(n)` — reads exactly N bytes
- [x] Implement `read_value()` — reads marker char, dispatches to correct read_* method
- [x] Implement `read_simple()` — reads `+{line}\r\n` → `SimpleString(line)`
- [x] Implement `read_error()` — reads `-{line}\r\n` → `Error(line)`
- [x] Implement `read_integer()` — reads `:{n}\r\n` → `Integer(n)`
- [x] Implement `read_bulk()` — reads `$-1\r\n` → `Null`, or `$N\r\n{N bytes}\r\n` → `BulkString(N bytes)`
- [x] Implement `read_array()` — reads `*N\r\n` → recursively decode N values → `Array(vec)`
