...
fn test_scenario_population_2000_users() {
    ...
    for i in 0..count {
        ...
        let jti_value = format!(
            r#"{{"jti":"{}","sub":"{}","sid":"{}","family_id":"{}"}}"#,
            refresh_key.split(':').nth(1).unwrap(), uid, sid, family_id
        );
        let session_json = format!(r#"{{"sid":"{}","jti":"{}"}}"#, sid, refresh_key.split(':').nth(1).unwrap());
        let session_key = format!("session:{}", sid);
        let family_key = format!("family:{}", family_id);

        // HSET refresh:{jti} 8 fields
        client.execute::<()>(client.hset(&refresh_key, "jti", refresh_key.split(':').nth(1).unwrap()));
        client.execute::<()>(client.hset(&refresh_key, "sub", &uid));
        client.execute::<()>(client.hset(&refresh_key, "sid", &sid));
        client.execute::<()>(client.hset(&refresh_key, "family_id", &family_id));
        client.execute::<()>(client.hset(&refresh_key, "iat", i.to_string()));
        client.execute::<()>(client.hset(&refresh_key, "exp", (i + 3600).to_string()));
        client.execute::<()>(client.hset(&refresh_key, "client_id", "web-app"));
        client.execute::<()>(client.hset(&refresh_key, "scopes", "read write"));

        // SADD family:{family_id} {jti}
        client.execute::<usize>(client.sadd(&family_key, refresh_key.split(':').nth(1).unwrap()));

        // SETEX session:{sid} 2592000 session_json
        client.execute::<()>(client.setex(&session_key, 2592000, &session_json));
    }
    ...
    assert!(
        elapsed_ms < 5000.0,
        "Population took {:.0}ms, target < 5000ms",
        elapsed_ms
    );
    ...
    for chunk in (0..count).step_by(chunk_size) {
        ...
        let handle = go!(move || -> usize {
            ...
            for i in chunk..end {
                ...
                let _ = client.execute::<()>(client.setex(&session_key, 2592000, &session_json));
                keys += 10;
            }
            keys
        });
...
fn test_scenario_login_burst() {
    ...
    for i in 0..login_count {
        ...
        let _ = client.execute::<()>(client.hset(&refresh_key, "family_id", &family_id));
        let _ = client.execute::<usize>(client.sadd(&family_key, &jti));
        let _ = client.execute::<()>(client.setex(&session_key, 2592000, &jti));
    }

    let elapsed_ms = start.elapsed().as_secs_f64() * 1000.0;
    let _ops_per_sec = (login_count as f64 / elapsed_ms) * 1000.0;
...
fn test_scenario_token_refresh_storm() {
    ...
    let elapsed_ms = start.elapsed().as_secs_f64() * 1000.0;
    let _ops_per_sec = (refresh_count as f64 / elapsed_ms) * 1000.0;
...
fn test_scenario_authz_load_10k_requests() {
    ...
    let elapsed_ms = start.elapsed().as_secs_f64() * 1000.0;
    let _ops_per_sec = (request_count as f64 / elapsed_ms) * 1000.0;
    let hit_ratio = (hits as f64 / request_count as f64) * 100.0;

    assert!(
        hit_ratio > 90.0,
        "Expected hit ratio > 90%, got {:.1}%",
        hit_ratio
    );
...
fn test_scenario_mixed_workload() {
    ...
    let elapsed_ms = start.elapsed().as_secs_f64() * 1000.0;
    let _ops_per_sec = (total_requests as f64 / elapsed_ms) * 1000.0;
